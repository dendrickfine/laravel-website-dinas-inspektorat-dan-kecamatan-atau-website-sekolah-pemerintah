			...
			<div class="card full-height">
				<div class="card-header">
					<div class="card-head-row">
						<div class="card-title">Edit Sambutan Playlist {{ $playlist->judul_playlist }}</div>
                        <a href="{{ route('playlist.index') }}" class="btn btn-primary btn-sm ml-auto">Kembali</a>
					</div>
				</div>
				<div class="card-body">
                    <form action="{{ route('playlist.update', $playlist->id_playlist) }}" method="POST" enctype="multipart/form-data">
					@csrf
                    @method('PUT')
